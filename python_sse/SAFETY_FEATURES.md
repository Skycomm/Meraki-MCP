# Cisco Meraki MCP Server (SSE Version) - Safety Features
...
- `src/utils/helpers.py` - Core confirmation and logging functions
- `src/config.py` - Environment variable configuration
- `src/tool_registry.py` - Integration with destructive operations
...
For SSE testing:
1. Always set `MCP_READ_ONLY_MODE=true` first
2. Test with a development organization
3. Review the audit log after testing
4. Only disable read-only mode when ready for production