...
def register_update_operations_tools():
    ...
    @app.tool(
        ...
        description="📡⚙️ Update general wireless settings for network"
    )
    def update_network_wireless_settings(
        ...
        named_vlans_pool_dhcp_monitoring: Optional[bool] = None
    ):
        ...
        try:
            ...
            if meshing_enabled is not None:
                kwargs['meshingEnabled'] = meshing_enabled
            if ipv6_bridge_enabled is not None:
                kwargs['ipv6BridgeEnabled'] = ipv6_bridge_enabled
            if location_analytics_enabled is not None:
                kwargs['locationAnalyticsEnabled'] = location_analytics_enabled
            if led_lights_on is not None:
                kwargs['ledLightsOn'] = led_lights_on
            if named_vlans_pool_dhcp_monitoring is not None:
                kwargs['namedVlansPoolDhcpMonitoring'] = named_vlans_pool_dhcp_monitoring
            
            result = meraki_client.dashboard.wireless.updateNetworkWirelessSettings(
                network_id, **kwargs
            )
            
            response = f"✅ Updated wireless settings\n"
            response += f"- Meshing: {'✅' if result.get('meshingEnabled') else '❌'}\n"
            response += f"- IPv6 Bridge: {'✅' if result.get('ipv6BridgeEnabled') else '❌'}\n"
            response += f"- Location Analytics: {'✅' if result.get('locationAnalyticsEnabled') else '❌'}\n"
