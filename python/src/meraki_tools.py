"""All 97 Meraki tools ported from the original MCP server"""
...
async def get_organization_uplinks_loss_and_latency(org_id: str, timespan: int = 300) -> str:
    """🚨 Get REAL packet loss and latency for all organization uplinks (5 minutes)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def update_network_appliance_firewall_l3_rules(network_id: str, 
    ...
    """🔥 Update Layer 3 firewall rules (BE CAREFUL!)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def confirm_reboot_device(serial: str, confirmation: str) -> str:
    """🔴 CONFIRM and execute device reboot (use with extreme caution)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def update_organization_license(org_id: str, license_id: str, device_serial: str = None) -> str:
    """📝 Update license assignment (assign to device)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def create_device_ping_test(serial: str, target: str, count: int = 5) -> str:
    """🏓 Run a ping test from a device (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def create_device_throughput_test(serial: str, target_serial: str) -> str:
    """🚀 Run throughput test between devices (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_device_throughput_test(serial: str, test_id: str) -> str:
    """🚀 Get throughput test results (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def create_switch_cable_test(serial: str, port: str) -> str:
    """🔌 Run cable diagnostic test on switch port (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def create_device_wake_on_lan(serial: str, vlan_id: int, mac_address: str) -> str:
    """⏰ Send Wake-on-LAN to device (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def create_switch_mac_table(serial: str) -> str:
    """📋 Get MAC address table from switch (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def blink_device_leds(serial: str, duration: int = 30) -> str:
    """💡 Blink device LEDs for identification (beta)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_device_memory_history(serial: str, timespan: int = 3600) -> str:
    """💾 Get device memory utilization history (2025 feature)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_device_cpu_power_mode_history(serial: str, timespan: int = 3600) -> str:
    """⚡ Get wireless device CPU power mode history (2025 feature)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_device_wireless_cpu_load(serial: str) -> str:
    """📊 Get wireless device CPU load monitoring (2025 feature)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_organization_switch_ports_history(org_id: str, timespan: int = 3600) -> str:
    """🔌 Get organization-wide switch port history (2025 feature)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_organization_devices_migration_status(org_id: str) -> str:
    """🔄 Get device migration status across organization (2025 feature)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def get_network_sm_devices(network_id: str) -> str:
    """📱 List all Systems Manager (MDM) devices in a network"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
...
async def confirm_reboot_network_sm_devices(network_id: str, device_ids: str, confirmation: str) -> str:
    """🔴 CONFIRM and execute SM device reboot (use with extreme caution)"""
    if not meraki_client:
        return "Error: Meraki client not initialized"
